...
const Profile = () => {
    ...
    const fetchData = async () => {
      ...
      try {
        const blogsResponse = await axios.get(
          "https://mern-blog-api-0v3s.onrender.com/get/user/blogs",
          { params: { email } }
        );
  ...
  const handleDeleteBlog = async (blogId) => {
    if (window.confirm("Are you sure you want to delete this blog??🥺")) {
      try {
        await axios.delete(`https://mern-blog-api-0v3s.onrender.com/delete/blogs/${blogId}`);
  ...
  const handleEditBlogSubmit = async (e) => {
    ...
    try {
      const response = await axios.put(
        `https://mern-blog-api-0v3s.onrender.com/put/blogs/${editingBlog._id}`,
        editingBlog
      );
      setBlogs((prev) =>
